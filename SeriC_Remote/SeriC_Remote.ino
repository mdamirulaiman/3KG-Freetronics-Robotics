...
int TurnSpeed, TurnSpeed_raw; // Left and Right Forward Turning Speed
...
void startRoutine() {
  ...
  if (digitalRead(Start)) {
    ...
    } else if (digitalRead(DS1) == 1 && digitalRead(DS2) == 1 && digitalRead(DS3) == 0) { //Front wait attack (001)
...
void setup() {
  ...
  while (millis() < 3000 ) {
    ...
    if (digitalRead(Start)) { // Button is pressed
...
void sensordebug() {
  int speed = analogRead(SPD);
  int turn = analogRead(TRN);
  Serial.print("Start Button : "); Serial.print(digitalRead(Start)); Serial.print("   ");
  Serial.print("Dipswitch : "); Serial.print(digitalRead(DS1)); Serial.print(digitalRead(DS2)); Serial.print(digitalRead(DS3)); Serial.print("\t");
